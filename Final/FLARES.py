...
def select_super_and_random_nodes(nodes, num_super, num_random, seed=42):
    """
    Select super nodes and random nodes based on weighted ranking.
    """
    np.random.seed(seed)
    logger.debug(f"Supernode seed: {seed}")

    if (num_super == 0):
        nodes = np.array(nodes)
        random_nodes = nodes[np.random.choice(len(nodes), size=min(num_random, len(nodes)), replace=False)].tolist()
    ...
    weights = {
        ...
        "mining_duration": 0.05,  # Lower is better, hence a penalty
        "selection_count": 0.5  # More selections are worse to ensure fairness
    }

    # Compute scores for all nodes
    ranked_nodes = sorted(nodes, key=lambda n: (compute_node_score(n, weights), n[0]), reverse=True)

    logger.debug(f"Ranked nodes: {str([(i[0]) for i in ranked_nodes])}")
    logger.debug(f"Scores: {str([(i[0], compute_node_score(i, weights)) for i in nodes])}")

    super_nodes = ranked_nodes[:num_super]
    remaining_nodes = ranked_nodes[num_super:]

    remaining_nodes_array = np.array(remaining_nodes)

    # Select `num_random` rows randomly
    random_nodes = remaining_nodes_array[np.random.choice(len(remaining_nodes),
                                                           size=min(num_random, len(remaining_nodes)),
                                                           replace=False)].tolist()

    res_super = [i[0] for i in super_nodes]
    res_rand = [i[0] for i in random_nodes]

    res = res_super + res_rand
...
def run(node: AICons.main.Node, dataset_subsets, scaler, logger, genesis_winnner):
    ...
    node_readings, super_random_nodes = [], []
    models, losses = [], []
    ...
    while True:
        try:
            ...
            if (block_iter == 0):
                logger.debug("INIT")
                super_random_nodes = select_super_and_random_nodes(NODE_PORTS, 0, NUM_SUPER + NUM_RANDOM, random.randint(1, 1000))
                if node.id in super_random_nodes:
                    node.selection_time += 1
            elif (block_iter % 10 == 0):
                logger.debug("NEW NODES")
                super_random_nodes = select_super_and_random_nodes(node_readings, NUM_SUPER, NUM_RANDOM, random.randint(1, 1000))
                if node.id in super_random_nodes:
                    node.selection_time += 1

            logger.info(f"Super Random nodes: {super_random_nodes}")
            ...
            if node.id in super_random_nodes:
                if block_iter % 10 == 0:
                    ...
                    expected_responses = len(super_random_nodes) - 1
                    ...
                    final_models = {k: 0 for k in super_random_nodes}
                ...
                if block_valid:
                    ...
                    node_metrics = {x: {'loss': losses[x], 'accuracy': model_accs[x]} for x in super_random_nodes if final_models[x] > n / 2}
                ...
                # Wait for potential new block from winner nodes
                logger.debug(f"Node {node.id}: Waiting for new blocks")
                new_blocks = wait_for_responses(shared_data, "NEW_BLOCK", 1)
